...
const DesignPage = () => {
  ...
  const filteredProducts = activeFilter === 'all' 
    ? designProducts 
    : designProducts.filter(p => p.category === activeFilter);
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="pt-32 md:pt-24 pb-8 px-4 max-w-7xl mx-auto page-transition">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-foreground">Jasa Desain</h1>
            <p className="text-muted-foreground">Desain profesional untuk kebutuhan Anda</p>
          </div>
        </div>
            ...
            <button
              ...
              className={`flex-shrink-0 px-5 py-2.5 font-medium rounded-xl transition-all ${
                activeFilter === filter.key
                  ...
                  : 'bg-card text-muted-foreground border border-border hover:border-brand-pink/30'
              }`}
            >
              {filter.label}
            </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredProducts.map((product) => (
            <ProductCard key={product.id} product={product} type="design" />
          ))}
        </div>
      </main>
    </div>
